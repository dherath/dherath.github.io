...
```
#### Computing the time for next event
...
```
From this point on you can either decide to draw the disks at each time step which will give a nice graphical output like in the _gif_ above Or more importantly you can visualize the position and velocity data of these disks(particles). First off lets look at the position data
